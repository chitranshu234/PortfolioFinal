    ...
    return (
        ...
        >
            ...
            <header className="staggered-menu-header" aria-label="Main navigation header">
                <div className="sm-logo" aria-label="Logo" onClick={() => { document.querySelector('#hero')?.scrollIntoView({ behavior: 'smooth' }); closeMenu(); }} style={{ cursor: 'pointer' }}>
                    {logoUrl ? (
                        <img src={logoUrl} alt="Logo" className="sm-logo-img" draggable={false} width={110} height={24} />
                    ) : (
                        <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '2rem', letterSpacing: '0.05em', color: '#c8c2b4' }}>
                            CP<span style={{ color: '#a09888' }}>.</span>
                        </span>
                    )}
                </div>
                <button
                        ...
                        {items && items.length ? (
                            items.map((it, idx) => (
                                <li className="sm-panel-itemWrap" key={it.label + idx}>
                                    <a className="sm-panel-item" href={it.link} aria-label={it.ariaLabel || it.label} data-index={idx + 1} onClick={(e) => {
